...
class StudentSubjectsScoreAPIView(APIView):
    ...
    def post(request, *args, **kwargs):
        ...
        if not all([first_name, last_name, subject_title, score]):
            return Response("Incomplete payload data", status=status.HTTP_400_BAD_REQUEST)

        try:
            score = float(score)
        except ValueError:
            return Response("Invalid score value", status=status.HTTP_400_BAD_REQUEST)

        if not (0 <= score <= 100):
            return Response("Score must be between 0 and 100", status=status.HTTP_400_BAD_REQUEST)

        try:
            credit = int(payload.get('credit'))
        except (ValueError, TypeError):
            return Response("Invalid credit value", status=status.HTTP_400_BAD_REQUEST)

        if not (1 <= credit <= 3):
            return Response("Credit must be between 1 and 3", status=status.HTTP_400_BAD_REQUEST)

        try:
            score_obj = Score.objects.get(student__first_name=first_name, student__last_name=last_name, subject__title=subject_title)
            ...
            if serializer.is_valid():
                serializer.save()
                score_obj = serializer.instance
            else:
                return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)

        data = {
            "student": {
                "first_name": score_obj.student.first_name,
                "last_name": score_obj.student.last_name
            },
            "subject_title": score_obj.subject.title,
...
class StudentSubjectsScoreDetailsAPIView(APIView):
    ...
    def get(request, *args, **kwargs):
        ...
        try:
            student = example_context_data.objects.get(id=student_id)
        except example_context_data.DoesNotExist:
        ...
        for score in scores:
            subject_data.append({
                "subject_title": score.subject.title,
                "credit": score.credit,
                "score": score.score,
                "grade": calculate_grade(score.score)
            })
            total_credit += score.credit
            total_grade_points += score.credit * score.score
...
class PersonnelDetailsAPIView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        personnel = data_pattern.objects.filter(school__title=school_name)

        data = []
        for index, person in enumerate(personnel, start=1):
            data.append(f"{index}. school: {person.school.title}, role: {person.role}, class: {person.class_order}, name: {person.first_name} {person.last_name}")
...
class SchoolHierarchyAPIView(APIView):
    ...
    def get(request, *args, **kwargs):
        ...
        for school_data in data_pattern:
            ...
            for class_key, class_data in school_data.items():
                if class_key != "school":
                    teacher = list(class_data.keys())[0]
                    head_of_room = class_data[teacher][0]["Head of the room"]
                    students = [student["Student"] for student in class_data[teacher][1:]]

                    school_structure["classes"].append({
                        "class": class_key,
                        "teacher": teacher,
                        "head_of_room": head_of_room,
                        "students": students,
                    })

            school_structure_list.append(school_structure)

        return Response(school_structure_list, status=status.HTTP_200_OK)
    ...
    def get(request, *args, **kwargs):
        ...
        def generate_structure(data):
            ...
            for item in data:
                title = item["title"]
                sub_data = item["sub"]
                sub_structure = generate_structure(sub_data)
                structure.append({"title": title, "sub": sub_structure})

            return structure

        your_result = generate_structure(data_pattern)
